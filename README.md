...
运行前需确认项目根目录存在 `小一17-25.xlsx`、`初一17-25.xlsx`、`2026小一录取规则.pdf`、`2026初一录取规则.pdf`。这些原始数据源已被 `.gitignore` 排除，不提交到仓库。
...
- 区域：布吉街道、布吉片区
- 年份：2017-2025
- 数据源：本地 `小一17-25.xlsx`、`初一17-25.xlsx`、`2026小一录取规则.pdf`、`2026初一录取规则.pdf`
...
- 无初中到小学映射表时，100 分以上高分民办初中优先使用高积分择校生源池；其它公办初中使用布吉公办小学均线，普通民办初中使用民办小学均线。
- 初中自身近年录取线趋势只作为辅助惯性参考；只有小学 cohort 信号缺失时才兜底使用。
- 试算积分和预测录取线均按 60-110 分区间处理。
- 预测只作为填报参考，不代表实际录取结果。
