...
运行前需确认项目根目录存在 `小一19-25.xlsx`、`初一19-25.xlsx`、`2025小一录取规则.pdf`、`2025初一录取规则.pdf`。这些原始数据源已被 `.gitignore` 排除，不提交到仓库。
...
- 区域：布吉街道、布吉片区
- 年份：2019-2025
- 数据源：本地 `小一19-25.xlsx`、`初一19-25.xlsx`、`2025小一录取规则.pdf`、`2025初一录取规则.pdf`
- 内容：录取分数线、初一民办直升分数线、2025 录取规则摘要与布吉相关 PDF 原文片段

## 积分试算与预测口径

- 积分试算按 2025 规则：先匹配基础类别，再按产权、租赁备案、学区户籍、社保月份等计算加分，加分合计按 10 分封顶。
