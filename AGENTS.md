...
- `小一19-25.xlsx`、`初一19-25.xlsx`、`2026小一录取规则.pdf`、`2026初一录取规则.pdf`：本地原始数据源，已被 `.gitignore` 排除，不提交到仓库。
...
使用本地原始 Excel/PDF 数据源重新生成 `data/admission-data.json` 和 `data/admission-data.js`。运行前需确认工作区根目录存在 `小一19-25.xlsx`、`初一19-25.xlsx`、`2026小一录取规则.pdf`、`2026初一录取规则.pdf`。
...
积分试算按 2026 规则执行，基础分加各项加分，加分合计封顶 10 分，试算总分和预测线均封顶 110 分。
